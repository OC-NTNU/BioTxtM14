...
The first results in LBD were produced by \cite{Swanson1986Fishoil} through manually executing a search algorithm based on co-occurrence statistics of terms. 
This allowed him to combine two publicly available knowledge fragments: (1) Fish oils reduce blood viscosity, (2) patients with Raynaud's disease tend to exhibit high blood viscosity. 
He combined these two to form the hypothesis that fish oils treat Raynaud's disease.
...
Most LBD methods are based on chaining of unspecified relations, using term co-occurrence frequencies as a heuristic for whether a relation holds between two terms.
...
This enables more complex query patterns.

LBD efforts along these lines in the biomedical domain have taken advantage of existing tools such as SemRep \cite{Rindflesch2003SemRep}.
...
Other work has concentrated on relation ond event extraction through machine learning. 
...
A machine learning approach has also been applied by \cite{Pechsiri2010a} to extract causal relations in the agricultural domain, which are then used to construct explanation knowledge graphs that represent the domain knowledge.
...
As an example of unsupervised techniques for causal relation extraction, \cite{Hashimoto2012Excitatory} propose a set of relations that can be used to detect causality. 
...
Other approaches have explored extraction of more fine-grained types of relations, including those of increase and decrease. 
\cite{Zambach2010Lexical} identify and linguistically analyse verbs that express regulation relations, positive and negative, between processes and substances in the biomedical domain. 
