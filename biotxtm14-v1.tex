...
Such discovery of implicit knowledge in a body of literature is the goal for the field of Literature-based discovery (LBD).
The first results in LBD were produced by \cite{Swanson1986Fishoil}, who combined two publicly available knowledge fragments: 1) Fish oils reduce blood viscosity, 2) patients with Raynaud's disease tend to exhibit high blood viscosity to form the hypothesis that fish oils treat Raynaud's disease.
...
The aim of LBD is to create systems that provide discovery support to uncover such potential hypotheses that Swanson referred to as \emph{undiscovered public knowledge} \cite{Swanson1986Undiscovered}.
Most LBD methods are based on chaining of underspecified relations, and use term co-occurrence frequencies as a heuristic whether a relation holds between two terms.
...
\newcite{Hristovski2008NLPinLBD} therefore advocate a text mining based approach, where the relation extraction is used to discover precise relations between two concepts.
This opens for more complex query patterns.
LBD efforts have traditionally been undertaken in the biomedical domain, and have therefore benefited from existing biomedical NLP tools such as SemRep (details below)
...
\EA{START CAUSAL RELATION EXTRACTION}
Causal relations is one of the most common target relations in text mining, and are used in a range of tasks, such as Question Answering.
...
Machine learning approach has also been applied by \cite{Pechsiri2010a} to extract causal relations in the agricultural domain. 
These relations are then used to construct explanation knowledge graphs that represent the domain knowledge.

A set of relations that can be used to detect causality  has been proposed by \cite{Hashimoto2012Excitatory}. 
...
Other approaches extract more fine-grained types of relations as well. 
In the work by \cite{Zambach2010Lexical}, the authors identify and analyse verbs that express regulation relations, positive and negative, between processes and substances in biomedical domain. 
They propose that this knowledge can be used for automatic annotation of such relations in text. 

SemRep is a major text mining system for the biomedical domain \cite{Rindflesch2003SemRep}. SemRep exploits structured domain knowledge found in the Unified Medical Language System (UMLS)\footnote{http://www.nlm.nih.gov/research/umls/}. The UMLS consists of three tools: A lexicon, a semantic network and a meta-thesaurus. Using underspecified symbolic language processing, SemRep is able to extracts a wide range of specific relations, such as TREATS, HAS\_PART and LOCATION\_OF.
\EA{END CAUSAL RELATION EXTRACTION}
...
It therefore seems unlikely that we can employ existing text mining tools.

As a first step towards developing text mining tools in this domain, we explore manual text annotation for creating annotated corpora, which can be used to train classifiers for automatic annotation, and ultimately automatic rule extraction.
