...
The annotation guidelines presented here have been developed with the goal of facilitating discovery of feedback loop candidates, in the way they focus on quantitative changes in variables rather than plain causal relations\EA{This sentence should maybe be moved to an earlier stage, as it explains our motivation. Also this is neglecting contradictions/other chains.}.
\EA{Feedback loops are normally expressed as a chain of change events where first and last concept are the same, however as there is no theoretical bound to the length of a feedback chain, it is impossible to enumerate the set of all patterns that can give rise to a feedback loop.
Reasoning will therefore follow a production rule inspired approach, where a feedback loop is detected if the system is able to infer a change from a working memory containing only the same changeJust stating my opinion, I know you might have entirely different conceptions of how the system will work}.

First, quantitative variables and the various types of events (i.e., change, causal, and correlation) and the correlation and causal relationships among these, as shown in annatation examples, are extracted and learned by the system.
Then the system will generate scientific hypotheses on the basis of this extracted knowledge. 
A hypothesis is either 1) a chain of events that either connects oceanographic entities that dont appear together in any of the extracted individual relationships, or 2) a chain that starts and ends with the same entity but does this through a chain of extracted relations of at least path length two. 
The latter type of chains capture hypotheses on the feedback mechanisms. 
Generation of hypotheses is essentially a search on a rather large event and relation space. 
Relations with the left-hand side (LHS) and right-hand side (RHS) representation can be considered as rules that can fire when their LHS matches the current focus of attention of the system. 
The focus is the active elements (quantitative variables, events, previously fired relations) that may be useful to pursue. 
Activation of these elements occur according to the RHS of a rule that is selected to fire. 
As time passes the number of activated elements increases quickly. 
To combat the increasing complexity of the search, some heuristic are needed. 
Heuristics need to be formulated according to the peculiarities of the climate domain. 
One of the concern may be whether the more general or the more specific rules will be most preferred. 
For example GRAFFITI Fajtlowicz (1988) that discovers hypotheses in geometry and conjectures in graph theory prefers more general statements over specific ones.
