...
One of the characteristics of the cross-disciplinary domain of earth science, environmental science and climate science is the existence of many different processes that affect each other in direct and indirect ways, resulting in highly complex systems.
...
Indentifying such feedback processes is generally considered a crucial step in understanding and predicting global warming.

Unfortunately much potential knowledge regarding processes and their interactions is likely to be hidden in the scientific literature, scattered over journals catering to different scientific communities with relatively little communication among them.
...
Earth, environmental and climate science, in particular, may all benefit from automatic retrieval and extraction of processes and their interactions from scientific publications.
...
An annotion scheme is proposed to annotate the events of interest, namely, those pf change, cause, correlation and feedback, as well as the entities involved in them, called quantitative variables.
Its purpose is to serve as an intermediary step in the process of rule extraction.
It is shown that such rules can indeed be automatically extracted from annotated text.
  
The remainder of this paper is structured as follows. 
The next Section first discusses related work on event extraction and caual relation extraction, as well as on literature-based knowledge discovery.
Section~\ref{sec:annot} proposes a new annotation scheme to capture the events of interest -- those of change, cause, correlation and feedback -- and the entities involved in them -- quantitative variables. 
In Section~\ref{sec:extraction} it is shown that rules can indeed be automatically extracted from annotated text.
...
Application of NLP-based LBD techniques to less resourced domains would however require the development or adaptation of NLP tools to the new domain.
...
On the other hand, \emph{export production} and \emph{biological pump} are different names, used by chemists and biologists repspectively, for the same process of carbon cycling in the oceans.
